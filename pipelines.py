...
def compute_all_models(language, input_words, fonts=None, n_letters=(5, 5), data_label=None, n_jobs=1):
	...
	if (fonts is None) or (fonts == 'word'):
		...
		for font in fonts:
			...
			orthope.run_all_oPEs(language=language, font=font, prior_font=font, input_words=input_words, n_letters=n_letters, data_label=data_label, n_jobs=n_jobs)
...
compute_all_models(language, input_words=input_words, fonts=fonts, n_letters=nletters_lims, data_label='gagl2020', n_jobs=n_jobs)
